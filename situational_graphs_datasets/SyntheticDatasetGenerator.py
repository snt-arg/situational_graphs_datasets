...
from situational_graphs_datasets.graph_visualizer import visualize_nxgraph, visualize_nxgraph_3d
from situational_graphs_datasets.NodeEdgeFeatureEmbeddingBuildier import NodeEdgeFeatureEmbeddingBuildier
...
class SyntheticDatasetGenerator():
                ...
                for ij_difference in [[1,0], [0,1]]:
                    ...
                    if current_room_id != -1.0 and comparison.all() and current_room_id != base_matrix[compared_ij[0],compared_ij[1]]:
                        ...
                        if compared_room_id != -1.0 and (current_room_id, compared_room_id) not in explored_walls:
                            ...
                            # print(f"Adding wall between rooms {current_room_id} and {compared_room_id} with limits {current_room_neigh_ws_center} and {compared_room_neigh_ws_center}")
                            graph.add_nodes([(node_ID,{"type" : "wall", "x" : wall_center, "center" : wall_center, "viz" : wall_viz})])
                            # print(f'Adding wall to ws {current_room_neigh_ws_id} and ws {compared_room_neigh_ws_id} with node ID {node_ID}')
    ...
    def add_door_nodes(self, graph, ratio=0.5):
        """
        Adds door nodes to the graph based on the existing wall surfaces.
        The doors are placed randomly along the walls, with a specified ratio of doors to walls.
        """
        # wall_nodes = graph.filter_graph_by_node_types("wall").get_nodes_ids()
        wall_nodes = list(graph.filter_graph_by_node_types("wall").get_nodes_ids())

        # print(f"Adding doors to {len(wall_nodes)} walls with a ratio of {ratio}")
        for wall_id in wall_nodes:
            # print(f"Adding door to wall {wall_id}")
            wall_data = graph.get_attributes_of_node(wall_id)
            neighbourhood_graph = graph.get_neighbourhood_graph(wall_id)
            edges = neighbourhood_graph.get_edges_ids()

            if np.random.rand() < ratio:
                # Place a door at a random position along the wall
                # door_position = wall_data["limit"][0] + np.random.rand() * (wall_data["limit"][1] - wall_data["limits"][0])
                door_position = wall_data["center"]
                door_id = max(graph.get_nodes_ids()) + 1
                
                viz_door_position = door_position + self.viz_center_offsets["object"]
                door_viz = copy.deepcopy(viz_data_base)
                door_viz.update({"type": "Point", "feat": 'g', "center": viz_door_position})

                # get ws in wall
                wall_ws_ids = graph.get_neighbourhood_graph(wall_id).filter_graph_by_node_types("ws").get_nodes_ids()

                graph.add_nodes([(door_id, {"type": "door", "x": door_position, "center": door_position, "viz": door_viz})])
                for ws_id in wall_ws_ids:
                    # print(f"Adding door {door_id} to wall surface {ws_id}")
                    graph.add_edges([(ws_id, door_id, {"type": "door_belongs_ws", "x": [], "viz_feat": "g", "linewidth": 1.0, "alpha": 0.5})])

        return graph

    def add_window_nodes(self, graph, ratio=0.5):
        """
        Adds window nodes to the graph based on the existing wall surfaces.
        The windows are placed randomly along the walls, with a specified ratio of windows to walls.
        """
        ws_nodes = list(graph.filter_graph_by_node_types("ws").get_nodes_ids())
        # print(f"Adding windows to {len(ws_nodes)} wall surfaces with a ratio of {ratio}")
        for ws_id in ws_nodes:
            neighbourhood_graph = graph.get_neighbourhood_graph(ws_id)
            # Check if not has a wall associated
            associated_walls = list(graph.get_neighbourhood_graph(ws_id).filter_graph_by_node_types("wall").get_nodes_ids())
            if associated_walls:
                continue  # Skip if the wall surface has an associated wall

            ws_data = graph.get_attributes_of_node(ws_id)
            if np.random.rand() < ratio:
                # Place a window at a random position along the wall surface
                # window_position = ws_data["limit"][0] + np.random.rand() * (ws_data["limit"][1] - ws_data["limits"][0])
                window_position = ws_data["center"]
                window_id = max(graph.get_nodes_ids()) + 1
                
                viz_window_position = window_position + self.viz_center_offsets["object"]
                window_viz = copy.deepcopy(viz_data_base)
                window_viz.update({"type": "Point", "feat": 'c', "center": viz_window_position})

                graph.add_nodes([(window_id, {"type": "window", "x": window_position, "center": window_position, "viz": window_viz})])
                graph.add_edges([(ws_id, window_id, {"type": "window_belongs_ws", "x": [], "viz_feat": "c", "linewidth": 1.0, "alpha": 0.5})])
    ...
    def get_filtered_graph(self, base_graph, node_types, full_edge_types):
        ...
        filtered_graph = filtered_graph.filter_graph_by_edge_types(full_edge_types[0])
        filtered_graph.to_directed()
    ...
    def extend_nxdataset(self, nxdataset, new_edge_type, stage):
        ...
        def apply_postprocess(self, pp_settings, working_graph):
            ...
            if pp_settings["pp_name"] == "filter":
                ...
                # Check if the filtered graph contains the correct edge types
                # for edge_type in pp_settings["edges"][0]:
                #     print(edge_type)
                #     edge_graph = working_graph.filter_graph_by_edge_types([edge_type])
                #     if edge_graph.get_edges_ids():
                #         print(f"Edge type {edge_type} found in the filtered graph.")
                        ...
                        if len(left_rooms) > 1 and np.random.random_sample() < pp_settings["room"]:
                            ...
                            for ws_node_id in ws_node_ids:
                                ...
                                for wall_ws_node_id in wall_ws_node_ids:
                                    ws_node_ids = list(working_graph.get_neighbourhood_graph(wall_ws_node_id).filter_graph_by_node_types(["ws"]).get_nodes_ids())
                                    if len(ws_node_ids) < 3:
                                        node_ids_selected.append(wall_ws_node_id)

                    working_graph.remove_nodes(node_ids_selected)

                    # Remove nodes linked to no node
                    door_node_ids = list(working_graph.filter_graph_by_node_types(["door"]).get_nodes_ids())
                    door_ids_selected = []
                    for door_node_id in door_node_ids:
                        door_node_neighbourhood = working_graph.get_neighbourhood_graph(door_node_id)
                        # print(f"Door {door_node_id} has neighbours {door_node_neighbourhood.get_nodes_ids()}")
                        if len(door_node_neighbourhood.get_nodes_ids()) < 2:
                            # print(f"Removing door {door_node_id} with no neighbours")
                            door_ids_selected.append(door_node_id)

                    window_node_ids = list(working_graph.filter_graph_by_node_types(["window"]).get_nodes_ids())
                    window_ids_selected = []
                    for window_node_id in window_node_ids:
                        window_node_neighbourhood = working_graph.get_neighbourhood_graph(window_node_id)
                        # print(f"Window {window_node_id} has neighbours {window_node_neighbourhood.get_nodes_ids()}")
                        # TODO(dps): Assuming that 1 node is the own node. Check if this can be filtered by node types
                        if len(window_node_neighbourhood.get_nodes_ids()) < 2:
                            # print(f"Removing window {window_node_id} with no neighbours")
                            window_ids_selected.append(window_node_id)
                    working_graph.remove_nodes(door_ids_selected + window_ids_selected)
            ...
            #{"pp_name": "K_rand_neigh", "max": 0, "types":["ws"], "use_gt":true}
                ...
                if pp_settings["msd"]: 
                    print("Plotting graph with MSD normals. NOT WORKING NOW")
            ...
            elif pp_settings["pp_name"] == "add_door_nodes":
                # print(f"ADDING DOOR NODES")
                working_graph = self.add_door_nodes(working_graph, pp_settings["ratio"])

            elif pp_settings["pp_name"] == "add_window_nodes":
                working_graph = self.add_window_nodes(working_graph, pp_settings["ratio"])
