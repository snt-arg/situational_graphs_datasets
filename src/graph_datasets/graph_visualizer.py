...
def visualize_nxgraph_3d(graph, image_name, visualize_alone=False, include_node_ids=True, logger=None):
    ...
    # Ensure all node centers and viz_data are 3D
    def to_3d(arr):
        arr = np.array(arr)
        if arr.shape[-1] == 2:
            arr = np.append(arr, 0)
    ...
    for node_data in nodes_data:
        if node_data[1]["viz_type"] == "Point":
            ...
            ax.scatter(viz_data[0], viz_data[1], viz_data[2], marker='o', s=markersize*30, color=_mpl_color_from_feat(node_data[1]["viz_feat"]))
            tag_center = viz_data
        elif node_data[1]["viz_type"] == "Line":
            viz_data = np.array(node_data[1]["viz_data"])
            ...
            ax.plot(viz_data[:,0], viz_data[:,1], viz_data[:,2], color=_mpl_color_from_feat(node_data[1]["viz_feat"]), linewidth=linewidth)
